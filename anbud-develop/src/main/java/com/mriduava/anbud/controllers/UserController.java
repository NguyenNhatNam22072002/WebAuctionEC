...
public class UserController {
	...
	public ResponseEntity<User> register(@RequestBody User user) {
		var newuser = userService.registerUser(user.getName(), user.getEmail(), user.getPictureUrl(),
				user.getPassword());
	...
	public String storeauthcode(@RequestBody String code, @RequestHeader("X-Requested-With") String encoding,
		...
		try {
			tokenResponse = new GoogleAuthorizationCodeTokenRequest(new NetHttpTransport(),
					...
					CLIENT_SECRET, code, "http://localhost:9000").execute();
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 * user, HttpServletRequest req) { securityLogin(user.getEmail(),
	 * user.getPassword(), req); return ResponseEntity.ok("ok"); }
	 */
